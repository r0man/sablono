...
;; (prn (html-expand [:div#a.b.c.d]))

(deftest test-js
  (let [datum {:key "key" :name "name"}]
    (clojure.pprint/pprint (html-expand [:div#a.b.c.d]))
...
(comment
  ...
  *data-readers*
  ...
  (def datum {:key "xasasa" :name "asas"})
  ...
  (html-expand [:div {:id "a"}])
  (html-expand [:img {"id" "foo"}])

  '(js/React.DOM.li
    (cljs.core/clj->js {})
    (js/React.DOM.a (cljs.core/clj->js {:href "#show/1"}))
    (js/React.DOM.div
     (cljs.core/clj->js {:id "item1", :className ["class1" "class2"]})
     (js/React.DOM.span (cljs.core/clj->js {:className "anchor"}) "a"))))
